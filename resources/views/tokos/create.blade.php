...
<div class="card shadow-sm">
    <div class="card-header bg-primary text-white">
        <h4 class="mb-0"><i class="fas fa-plus-circle me-2"></i>Tambah Toko Baru</h4>
    </div>
    <div class="card-body">
        <form action="{{ route('tokos.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name_toko') is-invalid @enderror" id="name_toko" name="name_toko" value="{{ old('name_toko') }}" required placeholder="Masukkan nama toko...">
                @error('name_toko')
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('owner_id') is-invalid @enderror" id="owner_id" name="owner_id" required>
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('owner_id') == $user->id ? 'selected' : '' }}>{{ $user->username }}</option>
                    @endforeach
                </select>
                @error('owner_id')
                    <div class="invalid-feedback">
